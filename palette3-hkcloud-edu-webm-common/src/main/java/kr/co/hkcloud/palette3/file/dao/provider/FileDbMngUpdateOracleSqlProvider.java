...
//@formatter:off
/**
 * 파일 DB 관리 갱신 Oracle SQL Provider
 * Implements the ProviderMethodResolver on your provider class
 * 
 * @author leeiy
 *
 */
public class FileDbMngUpdateOracleSqlProvider implements ProviderMethodResolver
{
    /**
     * 다운로드 수 증가
    ...
    public static String updateDnlodCnt(final FileDbMngUpdateRequest fileDbMngUpdateRequest)
    {
        return new SQL() {{
            ...
            SET(
                    String.format("%1$s = NVL(%1$s, 0) + 1" , FILE_DB_MNG.COLUMN.PLT_FILE.DWNLD_CNT.name())
