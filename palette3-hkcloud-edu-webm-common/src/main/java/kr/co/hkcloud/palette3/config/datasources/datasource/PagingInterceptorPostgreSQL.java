...
public class PagingInterceptorPostgreSQL implements Interceptor {
    ...
    public Object intercept(Invocation invocation) throws Throwable {
        try {
            ...
            int pagesCnt = 0;
            try {
                if (((HashMap) parameterObject).get("ROW_CNT") != null)
                    rowCnt = (Integer) ((HashMap) parameterObject).get("ROW_CNT");
                if (((HashMap) parameterObject).get("PAGES_CNT") != null)
                    pagesCnt = (Integer) ((HashMap) parameterObject).get("PAGES_CNT");
            ...
            if (rowCnt > 0 && pagesCnt > 0) {
                ...
                BoundSql countSql = new BoundSql(ms.getConfiguration(), "SELECT COUNT(1) FROM (" + boundSql.getSql() + ") it_count ",
                    boundSql.getParameterMappings(), parameterObject);
                Object countObj = executor.query(ms, parameterObject, rowBounds, resultHandler, countKey, countSql).get(0);

                BoundSql listSql = new BoundSql(ms.getConfiguration(), "SELECT PAGING.* FROM ( SELECT " + ((HashMap) countObj).get("count")
                    + " as TWB_PAGING_TOT_COUNT ,* FROM (" + boundSql.getSql() + ") it_list ) PAGING LIMIT " + rowCnt + " OFFSET (("
                    + pagesCnt + " - 1) * " + rowCnt + ") ", boundSql.getParameterMappings(), parameterObject);
