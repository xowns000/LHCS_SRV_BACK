...
/**
 *  페이징관련 Interceptor (Oracle)
 */
@Slf4j
@Intercepts(
        @Signature(type = Executor.class, method = "query", args = {MappedStatement.class, Object.class, RowBounds.class, ResultHandler.class})
)
public class PagingInterceptorOracle implements Interceptor {
    ...
    public Object intercept(Invocation invocation) throws Throwable {
        try {
            ...
            if( rowCnt > 0 && pagesCnt > 0 )
            {
                ...
                BoundSql countSql = new BoundSql(ms.getConfiguration(), "SELECT COUNT(1) COUNT FROM (" + boundSql.getSql() + ") it_count ", boundSql.getParameterMappings(), parameterObject);
                Object countObj = executor.query(ms, parameterObject, rowBounds, resultHandler, countKey, countSql).get(0);

                BoundSql listSql = new BoundSql(ms.getConfiguration(), "SELECT PAGING.* FROM ( SELECT "+ ((HashMap)countObj).get("COUNT") +" as TWB_PAGING_TOT_COUNT , it_list.* FROM (" + boundSql.getSql() + ") it_list ) PAGING OFFSET (("+ pagesCnt  +" - 1) * " + rowCnt + ") ROWS FETCH NEXT " + rowCnt + " ROWS ONLY ", boundSql.getParameterMappings(), parameterObject);
