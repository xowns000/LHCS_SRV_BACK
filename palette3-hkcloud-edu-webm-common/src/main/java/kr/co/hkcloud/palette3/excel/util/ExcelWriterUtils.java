...
public class ExcelWriterUtils
{
    ...
    public ExcelWriterUtils(Workbook workbook, Map<String, Object> model, HttpServletRequest request, HttpServletResponse response) {
    ...
    {
        return (String) model.get(ExcelConstant.FILE_NAME.getName());
    }
    ...
    {
        return (List<String>) model.get(ExcelConstant.HEAD.getName());
    }
    ...
    {
        return (List<List<String>>) model.get(ExcelConstant.BODY.getName());
    }
    ...
    private enum FileNameEncoder
    {
        ...
        static {
            FILE_NAME_ENCODER_MAP = EnumSet.allOf(FileNameEncoder.class).stream().collect(Collectors.toMap(FileNameEncoder::getBrowser, FileNameEncoder::getEncodeOperator));
        }
