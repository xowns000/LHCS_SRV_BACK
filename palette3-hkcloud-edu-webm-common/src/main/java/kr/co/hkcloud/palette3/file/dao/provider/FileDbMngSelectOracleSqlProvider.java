...
// @formatter:off
/**
 * 파일 DB 관리 조회 Oracle SQL Provider
 * Implements the ProviderMethodResolver on your provider class
 * 
 * @author leeiy
 *
 */
public class FileDbMngSelectOracleSqlProvider implements ProviderMethodResolver
{
    /**
     * 저장소대상유형 조회
    ...
    public static String selectRepositoryTrgtTypeCd(final FileDbMngSelectRequest fileDbMngSelectRequest)
    {
        return new SQL() {{
            ...
            if(!StringUtils.isEmpty(fileDbMngSelectRequest.getCustcoId()))
            {
                WHERE(String.format("%s = #{custcoId}"  , FILE_DB_MNG.COLUMN.PLT_FILE.CUSTCO_ID.name()));
            }
