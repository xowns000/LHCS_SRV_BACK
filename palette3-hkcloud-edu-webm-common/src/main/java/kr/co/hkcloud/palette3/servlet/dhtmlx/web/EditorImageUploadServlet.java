 ...
 * 사용안함 - 20210623
 * 
 * @classDescription 에디터 파일업로드 처리
 * @version          1.0
 */
@Slf4j
//@WebServlet(urlPatterns = {"/servlet/EditorImageUploadServlet"})
...
//        gStrWorkGB = req.getParameter("WORK_GB");
//
//        //업로드된 파일정보 저장처리 호출
//        if(fileFlag) {
...
//            String fileUrl = "/Repository/" + gStrWorkGB + objUploadFileInfo.getString("PATH_DIR") + objUploadFileInfo.getString("FILENAME");
...
//        //String strWorkGB = req.getParameter("WORK_GB");						//업로드 업무구분
//        String strWorkGB = gStrWorkGB;						//업로드 업무구분
//        String strRepositoryTempDir = fileCmmnUtils.getWorkGbFilePath(strWorkGB, "dir");
...
//        String strYear = paletteUtils.getYearString();
//        String strMonth = paletteUtils.getMonthString();
//        String strDay = paletteUtils.getDayString();
//        String strUploadDir = "editor/" + strYear + "/" + strMonth + "/" + strDay + "/" + req.getParameter("WORK_GB_KEY");	//editor/년/월/일/업무구분키(시퀀스)
...
//
//                //이미지 파일인지 체크
//                String strExtList[] = {"bmp", "jpg", "jpeg", "png", "gif"};
//
...
//
//                    if(!Arrays.asList(strExtList).contains(strFileExt)) {	//이미지 확장자가 아닐 경우
//                                                                         //이미지 확장자가 아님
//                        objUploadFileInfo.setHeader("ERROR_FLAG", true);
...
//                    strRealFileName = paletteUtils.getTimeStampMilis() + "_" + strWorkGB + "_" + strUserID + "." + strFileName.substring(strFileName.lastIndexOf(".") + 1);		//실제저장보관될파일명
//
//                    try {
//                        //파일생성
...
//                    objUploadFileInfo.setString("PATH_DIR", i, "/" + strUploadDir + "/");	//중간디렉토리 패스명
