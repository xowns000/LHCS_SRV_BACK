...
@Slf4j
@Component
public class ChatSettingProCounselByAgentManageValidator implements Validator {@Override
...
TelewebJSON objJsonParams = (TelewebJSON) target;
		...
		if (objJsonParams.getHeaderString("TYPE").toString().equals("BIZ_SERVICE")) {
			...
			if (objJsonParams.getHeaderString("METHOD").toString().equals("inqire")) {
				...
				{
					case "chat.setting.pro-counsel-by-agent-manage.inqry-ty-unasgn-user": {
						selectRtnUserInqryValidate(target, errors);
						break;
					}
					case "chat.setting.pro-counsel-by-agent-manage.unasgn-inqry-ty": {
						selectRtnUserInqryByUserValidate(target, errors);
						break;
					}
					default: // SERVICE값이 없을 때
					{
						...
						arg[0] = "SERVICE";
						errors.reject(PaletteValidationCode.UNDEFINED_SERVICE.getCode(), arg,
								PaletteValidationCode.UNDEFINED_SERVICE.getMessage());
						break;
					}
				}
			}
			else
			{
				Object[] arg = new Object[1];
				arg[0] = "METHOD";
				errors.reject(PaletteValidationCode.UNDEFINED_METHOD.getCode(), arg,
						PaletteValidationCode.UNDEFINED_METHOD.getMessage());
			}
		}
	...
	// 조회 유효성 검사
		public void selectRtnUserInqryValidate(Object target, Errors errors) {
			TelewebJSON objJsonParams = (TelewebJSON) target;

			if (StringUtils.isBlank(objJsonParams.getString("TALK_INQRY_CD"))) {
				Object[] arg = new Object[1];
				arg[0] = "TALK_INQRY_CD";
		...
		public void selectRtnUserInqryByUserValidate(Object target, Errors errors) {
			TelewebJSON objJsonParams = (TelewebJSON) target;
