...
VALUES(28, 'SYSMSG', 'SND', 'GENMSG', 0, '안녕하세요! 문의유형을 선택해주시면 채팅상담을 시작하겠습니다.', '문의유형 자동인사', NULL, 'Y', 2, TO_CHAR(NOW(),'YYYYMMDDHH24MISS'), 2, TO_CHAR(NOW(),'YYYYMMDDHH24MISS'));
INSERT INTO PLT_CHT_SYS_MSG
(sys_msg_id, msg_se_cd, rcptn_dsptch_cd, msg_type_cd, msg_hr, msg_cn, msg_expln, lnk_type_cd, use_yn, rgtr_id, reg_dt, mdfr_id, mdfcn_dt)
VALUES(29, 'SYSMSG', 'SND', 'GENMSG', 0, '안녕하세요! #{고객사}입니다!', '자동인사', NULL, 'Y', 2, TO_CHAR(NOW(),'YYYYMMDDHH24MISS'), 2, TO_CHAR(NOW(),'YYYYMMDDHH24MISS'));
...
VALUES((SELECT MAX(MENU_ID) FROM PLT_MENU) + 1, (SELECT MENU_ID FROM PLT_MENU WHERE PATH_NM = '/STA_M0300'), NULL, 3, 'SNS상담종합통계', '월별 SNS상담 이력 정보를 SNS별, 상담시간별, 일별 추이 현황에 대한 통계를 확인할 수 있습니다', NULL, NULL, 'MAIN', '/STA_M0306', NULL, NULL, '', NULL, NULL, NULL, NULL, NULL, NULL, NULL, 'Y', 'N', 1, 2, TO_CHAR(NOW(),'YYYYMMDDHH24MISS'), 2, TO_CHAR(NOW(),'YYYYMMDDHH24MISS'));
